...
@implementation Maintenance_11s
...
-(void)createUI{
    ...
    question.frame = CGRectMake(0, headView.frame.origin.y+headView.frame.size.height+10, screenRect.size.width, 60);
    question.textAlignment = NSTextAlignmentCenter;
    question.text = @"Select the picture of Repaired\nHeader Acrylic";
    ...
    if(screenRect.size.width<=320)
        profilePhoto.frame=CGRectMake(screenRect.size.width/2-50, question.frame.origin.y+question.frame.size.height+20, 100, 100);
    else
        profilePhoto.frame=CGRectMake(screenRect.size.width/2-75, question.frame.origin.y+question.frame.size.height+30, 150, 150);
...
//    }else{
        ...
        [[NSUserDefaults standardUserDefaults] setObject:UIImageJPEGRepresentation(profilePhoto.image, .3) forKey:@"HeaderAcylicPhoto"];
        [[NSUserDefaults standardUserDefaults]synchronize];
        
        
         Maintenance_12 * maintenance_12 = [[Maintenance_12 alloc]init:_doorName Doorid:_doorID];
            [self.navigationController pushViewController:maintenance_12 animated:YES];
