...
@interface Maintenance_12 ()
{
    ...
    UITextField* drawerQuanty;
    float textFieldWidth;
}
@end

@implementation Maintenance_12
...
-(void)createUI{
    ...
    question.text = @"Has Drawer Rails repaired ?";
    ...
    drawerQuanty=[[UITextField alloc]initWithFrame:CGRectMake(20,radioBtnNo.frame.origin.y+radioBtnNo.frame.size.height+30, scrollView.frame.size.width-40, textFieldWidth)];
    drawerQuanty.delegate=self;
    drawerQuanty.layer.cornerRadius=5;
    drawerQuanty.hidden= YES;
    drawerQuanty.backgroundColor=[UIColor colorWithRed:(CGFloat)230/255 green:(CGFloat)230/255 blue:(CGFloat)230/255 alpha:1];
    drawerQuanty.placeholder=@"Enter Drawer Rail Quantity";
    drawerQuanty.leftView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"textfieldBorder"]];
    drawerQuanty.font=[UIFont fontWithName:fontName size:15];
    drawerQuanty.keyboardType=UIKeyboardTypeNumberPad;
    drawerQuanty.leftViewMode = UITextFieldViewModeAlways;
    [scrollView addSubview:drawerQuanty];
...
- (void)tapReceived:(UITapGestureRecognizer *)tapGestureRecognizer{
    [drawerQuanty resignFirstResponder];// This will dismiss your keyBoard.
}
...
-(void)nextAction{
    ...
    [[NSUserDefaults standardUserDefaults] setObject:drawerRail forKey:@"DrawerRail"];
    ...
    if(yes){
        ...
        if([drawerQuanty.text isEqual:@""])
            [self.view makeToast:@"Bountique Quantity Missing!"];
        else{
            
            
            [[NSUserDefaults standardUserDefaults]setObject:drawerQuanty.text forKey:@"drawerRailQuantity"];
            [[NSUserDefaults standardUserDefaults]synchronize];
            
            Maintenance_15 * maintenance_15 = [[Maintenance_15 alloc]init:_doorName Doorid:_doorID];
            [self.navigationController pushViewController:maintenance_15 animated:YES];
